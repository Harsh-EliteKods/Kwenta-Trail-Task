...
import { AssetDisplayByAsset, MarketKeyByAsset, getDisplayAsset, formatDollars } from '@kwenta/sdk/utils';
...
const FuturesMarketsTable: React.FC<FuturesMarketsTableProps> = ({ search }) => {
  ...
  const data = useMemo(() => {
    ...
    return markets.map(market => {
      ...
      const pastPrice = pastRates.find(price => price.synth === getDisplayAsset(market.asset)?.toUpperCase());
      ...
      return {
        ...
        priceChange: pastPrice?.rate && marketPrice.gt(0) ? marketPrice.sub(pastPrice?.rate).div(marketPrice) : wei(0),
            ...
            columns={[
              {
                ...
                cell: cellProps => {
                  return (
                    <MarketContainer>
                      <IconContainer>
                        <StyledCurrencyIcon currencyKey={MarketKeyByAsset[cellProps.row.original.asset]} />
                      </IconContainer>
                      <StyledText>
              ...
              {
                header: () => <TableHeader>{t('dashboard.overview.futures-markets-table.mark-price')}</TableHeader>,
                accessorKey: 'price',
                cell: cellProps => {
                  return (
              ...
              },
              {
                ...
                cell: cellProps => {
                  return (
                    <ChangePercent value={cellProps.row.original.priceChange} decimals={2} className="change-pct" />
                  );
                },
                size: 105,
                enableSorting: true,
              ...
              },
              {
                ...
                cell: cellProps => {
                  return <Currency.Price price={cellProps.row.original.volume} formatOptions={{ truncateOver: 1e3 }} />;
                },
                size: 125,
                enableSorting: true,
        ...
        <StyledMobileTable
          ...
          columns={[
                ...
                return (
                  <div>
                    <MarketContainer>
                      <IconContainer>
                        <StyledCurrencyIcon
                          currencyKey={MarketKeyByAsset[cellProps.row.original.asset as FuturesMarketAsset]}
                        />
                      </IconContainer>
                      <StyledText>{cellProps.row.original.market}</StyledText>
                      <Currency.Price price={cellProps.row.original.price} formatOptions={{ suggestDecimals: true }} />
                    </MarketContainer>
                  </div>
                );
              },
            ...
            },
            {
              header: () => (
                <div>
                  <TableHeader>{t('dashboard.overview.futures-markets-table.open-interest')}</TableHeader>
                  <TableHeader>{t('dashboard.overview.futures-markets-table.funding-rate')}</TableHeader>
                </div>
              ),
              accessorKey: 'openInterest',
              cell: cellProps => {
                return (
                  <div>
                    <Currency.Price price={cellProps.row.original.openInterest} formatOptions={{ truncateOver: 1e3 }} />
                    <div>
                      <ChangePercent
                        showArrow={false}
                        value={cellProps.row.original.fundingRate}
            ...
            },
            {
              header: () => (
                <div>
                  <TableHeader>{t('dashboard.overview.futures-markets-table.daily-change')}</TableHeader>
                  <TableHeader>{t('dashboard.overview.futures-markets-table.daily-volume')}</TableHeader>
                </div>
              ),
              accessorKey: 'dailyVolume',
              cell: cellProps => {
