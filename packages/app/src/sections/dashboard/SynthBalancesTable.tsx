...
const conditionalRender = <T,>(prop: T, children: ReactElement) => (!prop ? <Body>{NO_VALUE}</Body> : children);
...
const SynthBalancesTable: FC<SynthBalancesTableProps> = ({ exchangeTokens }) => {
  ...
  const synthTokens = useMemo(() => {
    return synthBalances.map(synthBalance => {
      ...
      const pastPrice = pastRates.find(price => price.synth === getDisplayAsset(currencyKey)?.toUpperCase());
      ...
      return {
        ...
        usdBalance,
        price,
        priceChange: currencyKey === 'sUSD' ? ZERO_WEI : price?.sub(pastPrice?.rate ?? ZERO_WEI).div(price),
      };
    });
  }, [pastRates, prices, synthBalances, synthsMap]);

  const data = [...exchangeTokens, ...synthTokens].sort((a, b) => sortWei(a.usdBalance, b.usdBalance, 'descending'));
      ...
      <MobileHiddenView>
        <div>
          <Table
            ...
            noResultsMessage={<TableNoResults>{t('dashboard.overview.synth-balances-table.no-result')}</TableNoResults>}
            columns={[
              {
