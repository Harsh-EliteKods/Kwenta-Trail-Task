...
const TransferInputModal: FC<Props> = memo(({ onDismiss, totalEntries, totalAmount }) => {
  ...
  const onChange = useCallback((e: ChangeEvent<HTMLInputElement>) => setAddressOrName(e.target.value), []);
  ...
  return (
    <StyledBaseModal title={t('dashboard.stake.tabs.escrow.transfer-modal.title')} isOpen onDismiss={onDismiss}>
      ...
      <Spacer height={30} />
      <Body color="secondary">{t('dashboard.stake.tabs.escrow.transfer-modal.recipient-address')}</Body>
      <Spacer height={10} />
      <StyledInput value={recipient} onChange={onChange} />
      <Spacer height={30} />
      <FlexDivCol rowGap="8px">
        <FlexDivRowCentered>
          <Body color="secondary">{t('dashboard.stake.tabs.escrow.transfer-modal.entries-total')}</Body>
          <Body mono color="primary">
            {totalEntries.length}
          </Body>
        </FlexDivRowCentered>
        <FlexDivRowCentered>
          <Body color="secondary">{t('dashboard.stake.tabs.escrow.transfer-modal.amount-total')}</Body>
          <Body mono color="primary">
