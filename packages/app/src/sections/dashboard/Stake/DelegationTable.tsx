...
const DelegationTable = memo(() => {
  ...
  const [checkedState, setCheckedState] = useState(approvedOperators.map(() => false));
  ...
  useEffect(() => {
    setCheckedState(approvedOperators.map(() => false));
  }, [approvedOperators]);
  ...
  const handleOnSelectedRow = useCallback(
    (position: number) => {
      setCheckedState([...checkedState.map((_, index) => (index === position ? !checkedState[position] : false))]);
    },
    [checkedState],
  );
  ...
  const handleRevokeOperator = useCallback(
    (delegatedAddress: string) => {
      ...
      setCheckedState(data => data.map(() => false));
    },
    [dispatch],
  );
  ...
  return (
    <CardGridContainer>
      ...
      <StyledTable
        ...
        noResultsMessage={<TableNoResults>{t('dashboard.stake.tabs.delegate.no-result')}</TableNoResults>}
        columns={[
          {
            header: () => <></>,
            cell: cellProps => (
          ...
          },
          {
            ...
            cell: cellProps => (
              <TableCell>
                {lessThanWidth('md') ? truncateAddress(cellProps.getValue(), 15, 15) : cellProps.getValue()}
              </TableCell>
            ),
          },
        ]}
