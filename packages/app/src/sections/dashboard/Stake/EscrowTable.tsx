...
const EscrowTable = () => {
  ...
  const [checkedState, setCheckedState] = useState(escrowData.map(() => false));
  const [checkAllState, setCheckAllState] = useState(false);

  useEffect(() => {
    setCheckedState(escrowData.map(() => false));
  ...
  const selectAll = useCallback(() => {
    if (checkAllState) {
      ...
      setCheckedState(escrowData.map(() => true));
  ...
  const { ids, vestEnabled, transferEnabled } = useMemo(() => {
    ...
    const transferEnabled =
      ids.length > 0 && !STAKING_DISABLED && !stakingV1 && unstakedEscrowedKwentaBalance.gte(totalTransferAmount);
  ...
  const EscrowStatsContainer = () => (
    <StatsContainer columnGap="25px" justifyContent="flex-end">
      ...
      <ButtonContainer>
        ...
        <StyledButton variant="yellow" size="xsmall" isRounded disabled={!vestEnabled} onClick={handleOpenVestModal}>
    ...
    <EscrowTableContainer>
      <DesktopLargeOnlyView>
        <StyledTable
          ...
          noResultsMessage={<TableNoResults>{t('dashboard.stake.tabs.escrow.no-entries')}</TableNoResults>}
          columns={[
            {
              header: () => (
                <Checkbox
            ...
            },
            {
              ...
              cell: cellProps => (
                <FlexDivRowCentered columnGap="10px">
                  <StyledTableCell>{formatNumber(cellProps.row.original.amount, { minDecimals: 4 })}</StyledTableCell>
            ...
            },
            {
              ...
              cell: cellProps => (
                <StyledTableCell>{formatNumber(cellProps.row.original.vestable, { minDecimals: 4 })}</StyledTableCell>
              ),
                ...
                return (
                  <StyledTableCell color={common.palette.yellow.y500}>
                    {`${formatNumber(cellProps.row.original.fee, {
                      ...
                      cellProps.row.original.amount !== null ? fee.div(cellProps.row.original.amount) : ZERO_WEI,
                      { minDecimals: 0 },
                    )})`}
                  </StyledTableCell>
                );
        ...
        <StyledTable
          ...
          columns={[
            ...
            },
            {
              ...
              cell: cellProps => (
                <FlexDivRowCentered columnGap="10px">
                  <TableCell>{formatNumber(cellProps.row.original.amount, { minDecimals: 4 })}</TableCell>
                  {cellProps.row.original.version === 1 ? (
                    <StyledBadge color="yellow" size="small">
                      V1
                    </StyledBadge>
            ...
            {
              header: () => <TableHeader>{t('dashboard.stake.tabs.escrow.early-vest-fee')}</TableHeader>,
              cell: cellProps => {
                const fee = wei(cellProps.row.original.fee);
                return (
      ...
      {openModal === 'vest_escrow_entries' && (
        <VestConfirmationModal totalFee={totalFee} onDismiss={handleDismissModal} handleVest={handleVest} />
      )}
