      ...
      <DesktopOnlyView>
        <div>
          <Table
            ...
            onTableRowClick={row => router.push(ROUTES.Markets.MarketPair(row.original.market.asset, accountType))}
            noResultsMessage={
              !isL2 ? (
                <TableNoResults>
                  {t('common.l2-cta')}
            ...
            columns={[
              ...
              {
                header: () => <TableHeader>{t('dashboard.overview.futures-positions-table.avg-entry')}</TableHeader>,
                accessorKey: 'avgEntryPrice',
                cell: cellProps => {
                  return cellProps.row.original.activePosition.details?.avgEntryPrice === undefined ? (
                    <Body>{NO_VALUE}</Body>
                  ) : (
                    <CellContainer>
                      <Currency.Price
          ...
          <>
            ...
            <div style={{ margin: '0 15px' }}>
              {data.length === 0 ? (
                ...
                data.map(row => (
                  <MobilePositionRow
                    onClick={() => router.push(ROUTES.Markets.MarketPair(row.market?.asset ?? 'sETH', accountType))}
