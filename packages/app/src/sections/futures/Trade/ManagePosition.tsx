...
import { selectFuturesType, selectMarketIndexPrice, selectMarketPriceInfo } from 'state/futures/common/selectors';
...
const ManagePosition: React.FC = () => {
  ...
  const orderError = useMemo(() => {
    ...
    if (previewTrade?.statusMessage && previewTrade.statusMessage !== 'Success') return previewTrade?.statusMessage;
    return null;
  }, [previewTrade?.statusMessage, previewError, t]);

  const increasingPosition = !position?.activePosition.side || position?.activePosition.side === leverageSide;
  ...
  const otherReason = useMemo(() => {
    if (!isWalletConnected) {
      return {
        key: 'futures.market.trade.button.connect-wallet',
  ...
  const placeOrderDisabledReason = useMemo<{
    ...
    const invalidReason = orderPriceInvalidLabel(orderPrice, leverageSide, marketAssetRate, orderType);
  ...
  return (
    <>
      ...
      {!otherReason && placeOrderDisabledReason?.show ? (
        <Error message={placeOrderDisabledReason.message} messageType={placeOrderDisabledReason.show} />
      ) : null}
    </>
  );
};
