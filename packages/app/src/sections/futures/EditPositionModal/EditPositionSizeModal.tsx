...
import { editCrossMarginPositionSize, submitSmartMarginAdjustPositionSize } from 'state/futures/smartMargin/actions';
import {
...
export default function EditPositionSizeModal() {
  ...
  const isLoading = useMemo(() => isSubmitting || isFetchingPreview, [isSubmitting, isFetchingPreview]);
  ...
  const resultingLeverage = useMemo(() => {
    ...
    return position.remainingMargin?.gt(0) ? preview.size.mul(marketPrice).div(position.remainingMargin).abs() : wei(0);
  ...
  const invalid = useMemo(() => sizeWei.abs().gt(maxNativeValueWithBuffer), [sizeWei, maxNativeValueWithBuffer]);

  const previewError = useMemo(() => {
    if (preview?.exceedsPriceProtection && !overridePriceProtection) return 'Exceeds Price Protection';
    ...
    return null;
  }, [preview?.statusMessage, preview?.exceedsPriceProtection, maxLeverageExceeded, overridePriceProtection]);

  const errorMessage = useMemo(() => previewError || transactionState?.error, [previewError, transactionState?.error]);
  ...
  return (
    ...
    >
      ...
      <SegmentedControl values={['Increase', 'Decrease']} selectedIndex={editType} onChange={onChangeTab} />
      <Spacer height={20} />
      ...
      <Spacer height={6} />
      <InfoBoxContainer>
        <InfoBoxRow boldValue title={t('futures.market.trade.edit-position.market')} textValue={market?.marketName} />
        <InfoBoxRow
          textValueIcon={resultingLeverage && <PreviewArrow showPreview>{resultingLeverage.toString(2)}x</PreviewArrow>}
          ...
          textValueIcon={
            preview?.size && (
              <PreviewArrow showPreview>
                {position?.remainingMargin ? formatNumber(preview.size.abs(), { suggestDecimals: true }) : '-'}
              </PreviewArrow>
            )
          }
