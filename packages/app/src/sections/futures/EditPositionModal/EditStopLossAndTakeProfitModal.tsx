...
import { calculateKeeperDeposit, updateStopLossAndTakeProfit } from 'state/futures/smartMargin/actions';
...
export default function EditStopLossAndTakeProfitModal() {
  ...
  const [showOrderWarning, setShowOrderWarning] = useState(!stopLoss && !takeProfit && !hideOrderWarning);

  const hasInputValues = useMemo(() => takeProfitPrice || stopLossPrice, [takeProfitPrice, stopLossPrice]);
  ...
  const hasChangeOrders = useMemo(() => {
    const tpOrderPrice = takeProfit?.targetPrice ? stripZeros(takeProfit?.targetPrice?.toString()) : '';
  ...
  const ethBalWarningMessage = ethBalanceExceeded ? t('futures.market.trade.confirmation.modal.eth-bal-warning') : null;
  ...
  useEffect(() => {
    ...
    dispatch(setSLTPModalStopLoss(existingSL?.targetPrice ? stripZeros(existingSL.targetPrice.toString()) : ''));
    dispatch(setSLTPModalTakeProfit(existingTP?.targetPrice ? stripZeros(existingTP.targetPrice.toString()) : ''));
  ...
  const onSetStopLossAndTakeProfit = useCallback(() => dispatch(updateStopLossAndTakeProfit()), [dispatch]);
    ...
    >
      ...
      <InfoBoxRow
        ...
        nodeValue={
          <FlexDivRowCentered>
            ...
            <Spacer width={8} /> <PositionType side={position?.activePosition.side || PositionSide.LONG} />
          </FlexDivRowCentered>
        }
      />
      ...
      {showOrderWarning ? (
        ...
        <>
          ...
          <SelectorButtons onSelect={onSelectTakeProfit} options={TP_OPTIONS} type={'pill-button-large'} />
          ...
          <SelectorButtons onSelect={onSelectStopLossPercent} options={SL_OPTIONS} type={'pill-button-large'} />
