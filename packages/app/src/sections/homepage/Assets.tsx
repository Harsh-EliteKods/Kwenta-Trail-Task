...
export const PriceChart = ({ asset }: PriceChartProps) => {
  ...
  useEffect(() => {
    ...
    requestCandlesticks(getDisplayAsset(asset), Math.floor(Date.now() / 1000) - SECONDS_PER_DAY, undefined, 3600)
      .then(bars => {
        let positive = false;
        if (bars !== undefined) {
              ...
              {perps.map(({ key, name, description, price, volume, priceChange, image, icon }) => (
                <StatsCardContainer key={key} className={key}>
                  <StatsCard
                    noOutline
                    onClick={() => {
                      router.push(`/market/?asset=${key}&accountType=${DEFAULT_FUTURES_MARGIN_TYPE}`);
                    }}
                  >
