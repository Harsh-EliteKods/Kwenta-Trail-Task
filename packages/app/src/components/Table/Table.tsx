      ...
      <TableContainer>
        <ReactTable $rounded={rounded} $noBottom={noBottom} className={className}>
          ...
          {isLoading ? (
            ...
            <NoResultsContainer $padding={noResultsContainerPadding}>{noResultsMessage}</NoResultsContainer>
          ) : (
            <TableBody className="table-body">
              {table.getRowModel().rows.map((row, i) => {
                const localRef = lastRef && i === table.getState().pagination.pageSize - 1 ? lastRef : defaultRef;
                return (
                  <TableBodyRow
                    key={row.id}
                    localRef={localRef}
