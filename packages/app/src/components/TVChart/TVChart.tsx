...
export function TVChart({
  ...
  const decimals = Number(initialPrice) > 100 && Number(initialPrice) < 1000 ? 3 : suggestedDecimals(initialPrice);
      ...
      _widget.current?.chart().dataReady(() => {
        ...
        _oderLineRefs.current = openOrders.reduce((acc, order) => {
          if (order.targetPrice) {
            const color = colors.selectedTheme.chartLine[order.isSlTp ? 'default' : order.side ?? 'short'];
  ...
  useEffect(() => {
    ...
    const widgetOptions: ChartingLibraryWidgetOptions = {
      ...
      datafeed: DataFeedFactory((network?.id ?? chain.optimism.id) as NetworkId, chartScale, onSubscribe),
  ...
  useEffect(() => {
    if (onToggleShowOrderLines) {
      _toggleListener.current && _toggleLinesButton.current?.removeEventListener('click', _toggleListener.current);
      _toggleLinesButton.current?.addEventListener('click', onToggleShowOrderLines);
      _toggleListener.current = onToggleShowOrderLines;
    }
  }, [onToggleShowOrderLines]);
  ...
  useEffect(() => {
    _widget.current?.onChartReady(() => {
      ...
      _widget.current?.setSymbol(marketAsset + ':sUSD', symbolInterval?.interval ?? DEFAULT_RESOLUTION, () => {});
    });
  }, [marketAsset]);
