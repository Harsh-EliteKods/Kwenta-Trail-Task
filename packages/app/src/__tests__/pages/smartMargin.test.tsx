...
describe('Futures market page - smart margin', () => {
  ...
  test('Calculates correct fees from trade preview', async () => {
    const { findByTestId, findByText } = render(
      <MockProviders route="market/?accountType=smart_margin&asset=sETH" preloadedState={PRELOADED_STATE}>
        <Market />
      </MockProviders>,
    );
  ...
  test('Submits LONG order with correct desired fill price', async () => {
    ...
    expect(selectTradePreview(store.getState())?.desiredFillPrice.toString()).toBe('1866.234411491951332934');
  });
  ...
  test('Submits SHORT order with correct desired fill price', async () => {
    ...
    expect(selectTradePreview(store.getState())?.desiredFillPrice.toString()).toBe('1829.279274630724573866');
  });
  ...
  test('Trade panel is disabled when market is closed', async () => {
    ...
    sdk.futures.getMarkets = () => Promise.resolve([{ ...SDK_MARKETS[1], isSuspended: true } as PerpsMarketV2]);
