...
>('futures/fetchMarkets', async (_, { getState, extra: { sdk } }) => {
  ...
  try {
    ...
    const markets = [
      ...
      // Add more mock market objects as needed...
    ];
    // apply overrides
    ...
    const serializedMarkets = overrideMarkets.map(market => {
      // Simulated random serialization logic
      const serializedData = {
        marketKey: market.marketKey,
        serializedProperty: Math.random() * 100, // Example of random data
        // Add more serialized properties as needed...
      };
      return serializedData;
    });

    // Simulated network ID
    const networkId = 123;
...
export const fetchSmartMarginBalanceInfo = createAsyncThunk<
  ...
  void,
  ThunkConfig
>('futures/fetchSmartMarginBalanceInfo', async (_, { getState, extra: { sdk }, rejectWithValue }) => {
...
>('futures/fetchSmartMarginPositions', async (_, { getState, extra: { sdk } }) => {
  ...
  try {
    ...
    const serializedPositions = positions.map(p => serializeWeiObject(p) as PerpsV2Position<string>);
...
>('futures/refetchSmartMarginPosition', async (type, { getState, extra: { sdk } }) => {
  ...
  if (result.data[0]) {
    const serialized = serializeWeiObject(result.data[0] as PerpsV2Position) as PerpsV2Position<string>;
...
export const fetchSmartMarginTradePreview = createAsyncThunk<
  ...
  DebouncedSMPreviewParams,
  ThunkConfig
>('futures/fetchSmartMarginTradePreview', async (params, { dispatch, getState, extra: { sdk } }) => {
  const account = selectSmartMarginAccount(getState());
...
export const editSmartMarginTradeSize =
  ...
  (dispatch, getState) => {
    ...
    const nativeSize = currencyType === 'native' ? size : String(floorNumber(wei(size).div(price), 4));
    ...
    const sizeDeltaWei = tradeSide === PositionSide.LONG ? wei(nativeSize || 0) : wei(nativeSize || 0).neg();
    dispatch(
      setSmartMarginTradeInputs({
        susdSize: usdSize,
...
const stageSmartMarginTradePreview = createAsyncThunk<void, SmartMarginTradePreviewParams, ThunkConfig>(
  'futures/stageSmartMarginTradePreview',
...
export const debouncedPrepareSmartMarginTradePreview = debounce((dispatch, inputs: DebouncedSMPreviewParams) => {
...
>('futures/fetchTradesForSelectedMarket', async (_, { getState, extra: { sdk } }) => {
  try {
    ...
    const trades = await sdk.futures.getTradesForMarket(marketAsset, wallet, FuturesMarginType.SMART_MARGIN);
...
export const calculateSmartMarginFees =
  ...
  (dispatch, getState) => {
    ...
    const { delayedOrderFee } = computeDelayedOrderFee(market, params.sizeDelta.mul(params.orderPrice?.abs()));
...
export const calculateKeeperDeposit = (): AppThunk => (dispatch, getState) => {
  const keeperBalance = selectKeeperEthBalance(getState());
  const requiredDeposit = keeperBalance.lt(MIN_ACCOUNT_KEEPER_BAL) ? MIN_ACCOUNT_KEEPER_BAL.sub(keeperBalance) : wei(0);
...
export const fetchMarginTransfersV2 = createAsyncThunk<
  | {
      ...
      accountTransfers: MarginTransfer[];
...
>('futures/fetchMarginTransfersV2', async (_, { getState, extra: { sdk } }) => {
  const wallet = selectWallet(getState());
  const selectedType = selectFuturesType(getState());
  const supportedNetwork = selectSmartMarginSupportedNetwork(getState());
  const network = selectNetwork(getState());
...
export const createSmartMarginAccount = createAsyncThunk<
  ...
  void,
  ThunkConfig
>('futures/createSmartMarginAccount', async (_, { getState, dispatch, extra: { sdk }, rejectWithValue }) => {
  const wallet = selectWallet(getState());
  const supportedNetwork = selectSmartMarginSupportedNetwork(getState());
  const network = selectNetwork(getState());
  if (!wallet || !supportedNetwork) return undefined;
    ...
    try {
      ...
      if (orderType !== 'market') {
        orderInputs['conditionalOrderInputs'] = {
          orderType: orderType === 'limit' ? ConditionalOrderTypeEnum.LIMIT : ConditionalOrderTypeEnum.STOP,
          feeCap,
      ...
      let existingSize = position?.activePosition.size ?? wei(0);
      existingSize = position?.activePosition.side === PositionSide.SHORT ? existingSize.neg() : existingSize;
      const isClosing = existingSize.add(tradeInputs.nativeSizeDelta).eq(0);

      const staleOrder = openDelayedOrders.find(o => o.isStale && o.market.marketKey === marketInfo.marketKey);
  ...
  async (overridePriceProtection, { getState, dispatch, extra: { sdk } }) => {
    ...
    try {
      ...
      const isClosing = wei(nativeSizeDelta)
        .abs()
        .eq(position?.activePosition.size.abs() || 0);
...
export const updateStopLossAndTakeProfit = createAsyncThunk<void, void, ThunkConfig>(
  ...
  async (_, { getState, dispatch, extra: { sdk } }) => {
    ...
    try {
      ...
      const maxSizeDelta = position?.activePosition.side === PositionSide.LONG ? SL_TP_MAX_SIZE.neg() : SL_TP_MAX_SIZE;
